$(function () {
    ...
    $(document).on("click",".col",function(){
        let x = $(this);
        let checkbox = x.children(".col-check-content");
        // alert(checkbox.prop("checked"))
        if (checkbox.prop("checked") === false) {
            let index = x.children(".block").text();
            $.ajax({
                async: true,
                type:"GET",
    ...
    $(document).on("click", ".check-button", function () {
        ...
        $(".func-box-1 .c-2 ").css("display", "block");
        //获取区块hash：
...
function ch2(i) {
    if (i === 1) return "<span style='color: #05c405'>有效</span>";
            ...
            if (data.code === 200) {
                ...
                for (let i = 0; i < length; i++) {
                    $("#table-body").append("<div class=\"col\", id=\"col-" + (i+2) + "\">\n" +
                        "<input class=\"col-check-content\" type=\"checkbox\">" +
                        "                        <div class=\"PreviousHash  xb\">" + arrays[i].previousHash.substring(0,19) + "...</div>\n" +
